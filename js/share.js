...
_wxData.shareTitle = _wxData.shareTitle || $("title").html() || '精彩活动';
_wxData.shareDesc = _wxData.shareDesc || '点击了解更多';
_wxData.shareImgUrl = _wxData.shareImgUrl || location.protocol+'//img.intech.gdinsight.com/wk/lt_tca/images/share.jpg';
_wxData.shareLink = _wxData.shareLink || location.href.split("#")[0];

var _pWxShare = {
    xmlHttp: null,
    wxData: null,//保存微信分享数据
    //用来请求成签名的URL
    callback: null,
    createSignUrl: 'http://h5.szhhhd.com/wx/jssdk_insight_GD.php?url=',//生成签名的PHP程序
    /**
     * 初始化
     * url      : 生成签名的PHP程序 +当前的URL地址
     * callback : 回调函数
     */
    init: function (wxData, callback) {
        this.wxData = wxData || _wxData;
        this.callback = _wxData.callback;
        var _this = this;
        $.ajax({
            type: "GET",
            url: this.createSignUrl + escape(this.wxData.shareLink) + '&rd=' + Math.random(),
            dataType: "jsonp",
    ...
    handleStateChange: function (callback) {
     ...
     * @param  {[type]}   title    [标题]
     * @param  {[type]}   desc     [描述]
     * @param  {[type]}   imgUrl   [图片]
     * @param  {[type]}   link     [链接]
     * @param  {Function} callback [分享完成的回调]
     */
    share: function (wxData, callback) {
        var wxData = wxData || this.wxData;
        var callback = this.wxData.callback;
        var _title = wxData.shareTitle,
            _desc = wxData.shareDesc,
            _imgUrl = wxData.shareImgUrl,
            _link = wxData.shareLink,
            _this = this;

        function _wxReadyShare(callback) {
            //分享的数据
            var _wxData = {
                'title': _title,//分享的标题
                'link': _link,//分享的链接
                'desc': _desc,//描述信息
                'imgUrl': _imgUrl,//分享的图片
                //分享完成的回调
                //'success':wxData.success?wxData.success:function(res){
                ...
                _wxData_Timeline = {
                    ...
                    'success': function (res) {
                        ...
                        if (callback) {
                            //res.shareType = '分享到朋友圈';
                            callback('分享到朋友圈');
                        }
                    }
                };
            wx.onMenuShareAppMessage(_wxData);//分享给好友
            wx.onMenuShareQQ(_wxData_QQ);//分享到QQ
            wx.onMenuShareWeibo(_wxData_Weibo);//分享到微博
            wx.onMenuShareTimeline(_wxData_Timeline);//分享到朋友圈
        }
        if (this._isWxReady) {
            _wxReadyShare(callback);
        }
    }
}
_pWxShare.init();